...
inline void Blip_Buffer::end_frame( blip_time_t t ) {
	offset_ += t * factor_;
    // WAS: assert(( "Blip_Buffer::end_frame(): Frame went past end of buffer",samples_avail() <= (long) buffer_size_ ));
	assert(samples_avail() <= (long) buffer_size_ );
}

inline void Blip_Buffer::remove_silence( long count ) {
    
    // WAS: assert(( "Blip_Buffer::remove_silence(): Tried to remove more samples than available",count <= samples_avail() ));
